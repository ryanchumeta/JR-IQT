...
class Zaber:
    ...
    def move_absolute(self, axis, position, waitIdle=True):
        try:
            self.axes[axis].move_absolute(
                position,
                Units.LENGTH_MILLIMETRES,
                wait_until_idle=waitIdle,
                velocity=4,
                velocity_unit=Units.VELOCITY_MILLIMETRES_PER_SECOND,
            )
    ...
    def check_homed(self):
        return all([self.axes[i].is_homed() for i in range(3)])

    def move_absolute_async(self, x, y, z):
        self.axes[0].move_absolute(
            x,
            Units.LENGTH_MILLIMETRES,
